...
def update_line_figure_adaline(prototypes, classifications, figure, x, y):
    figure.cla()
    classified_0 = []
    classified_1 = []
    for i in range(len(prototypes)):
...
# Plot for the classification each prototype received
def update_class_figure_3D(prototypes, classifications, figure):
...
def update_plane_figure(prototypes, figure, classifications, x, y, z):
    figure.cla()
    classified_0 = []
    classified_1 = []
    for i in range(len(prototypes)):
...
def recall_figure_3D(prototypes, predicted_t, figure):
    classified_correct = []
    classified_incorrect = []
    for i in range(len(prototypes)):
        if predicted_t[i] == True:
            classified_correct.append([prototypes[i, 0], prototypes[i, 1], prototypes[i, 2]])
        else:
            classified_incorrect.append([prototypes[i, 0], prototypes[i, 1], prototypes[i, 2]])
    classified_correct = np.array(classified_correct)
    classified_incorrect = np.array(classified_incorrect)
    if len(classified_correct) != 0:
